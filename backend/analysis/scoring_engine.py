...
def _offer_component(offer: dict[str, Any]) -> dict[str, Any]:
    ...
    # İzahnamedeki halka açıklık yoksa gerçekleşen fiili dolaşım oranına düş.
    float_pct = _number(offer.get("float_pct"))
    if float_pct is None:
        float_pct = _number(offer.get("actual_float_pct"))
...
def _red_flags(offer: dict[str, Any]) -> list[str]:
    ...
    float_pct = _number(offer.get("float_pct")) or _number(offer.get("actual_float_pct"))
    if float_pct is not None and float_pct < 10:
        flags.append("⚠️ Halka açıklık oranı %10 altındadır; likidite riski yüksek olabilir.")

    # Ek satış (green shoe) arz büyüklüğünü kayda değer biçimde artırıyorsa
    # dağıtılacak lot beklentisi değişir; bu bir kalite değil kapsam uyarısıdır.
    extra_lots = _number(offer.get("extra_lots"))
    if total_lots and extra_lots and extra_lots / total_lots >= 0.15:
        flags.append(
            f"ℹ️ Ek satış hakkı arzı %{extra_lots / total_lots * 100:.0f} oranında büyütebilir; "
            "kişi başına düşen lot bu senaryoda değişir."
        )
