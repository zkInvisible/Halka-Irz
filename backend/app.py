...
#: Oy tablosu kurulumunu yeniden deneyen kapanış; veritabanı açılışta
#: erişilemezse zamanlayıcı bunu periyodik olarak tekrar çağırır.
vote_table_check = None
...
#: Rapor özeti de bellekte tutulur. Ölçümde /healthz her çağrıda Supabase'e
#: gidip 1,7–3,8 saniye sürüyordu; sağlık kontrolü ve durum sorgusu
#: veritabanına bağımlı olmamalı.
_meta_memo: dict[str, Any] = {"value": {}, "loaded_at": 0.0}
_meta_memo_lock = threading.Lock()
META_MEMO_TTL_SECONDS = 20


def invalidate_report_cache() -> None:
    with _report_memo_lock:
        _report_memo["loaded_at"] = 0.0
    with _meta_memo_lock:
        _meta_memo["loaded_at"] = 0.0


def cached_report_meta(*, allow_stale: bool = False) -> dict[str, Any]:
    """Rapor özetini kısa süreli bellekten döndürür.

    ``allow_stale`` ile veritabanına hiç gidilmez; sağlık kontrolü bunu kullanır
    çünkü orada birkaç saniyelik eskilik önemsiz, gecikme önemlidir.
    """
    with _meta_memo_lock:
        loaded_at = _meta_memo["loaded_at"]
        fresh = time.monotonic() - loaded_at < META_MEMO_TTL_SECONDS
        # Hiç yüklenmediyse eski veri de yok; o durumda bir kez okumak gerekir.
        if fresh or (allow_stale and loaded_at > 0):
            return _meta_memo["value"]

    meta = load_report_meta()
    with _meta_memo_lock:
        _meta_memo.update(value=meta, loaded_at=time.monotonic())
    return meta
...
def _data_age_hours(*, allow_stale: bool = False) -> float | None:
    return report_age_hours(cached_report_meta(allow_stale=allow_stale))
...
def background_scheduler() -> None:
    ...
    while True:
        ...
        # Veritabanı açılışta erişilemediyse geri geldiğinde kendiliğinden toparla.
        if vote_table_check is not None:
            vote_table_check()
...
def create_app() -> Flask:
    ...
    # Oy tablosunu kurmayı dene, ama başarısızlığı ölümcül sayma. Veritabanı
    # açılış anında erişilemezse (Supabase uykuda, şifre döndürülmüş, ağ kesik)
    # eski sürümde uygulama hiç açılmıyordu: site statik sayfayı bile
    # veremiyordu. Artık site ayakta kalır, oylar devre dışı olur ve bağlantı
    # geri geldiğinde zamanlayıcı tabloyu kurar.
    def ensure_vote_table() -> bool:
        nonlocal votes_ready, votes_retry_after
        if votes_ready:
            return True
        # Geri çekilme olmadan her istek ölü veritabanına bağlanmayı deneyip
        # saniyelerce bekletirdi.
        now = time.monotonic()
        if now < votes_retry_after:
            return False
        try:
            with app.app_context():
                db.create_all()
        except Exception as error:  # noqa: BLE001
            votes_retry_after = now + 30
            app.logger.warning("Oy tablosu kurulamadı: %s: %s", type(error).__name__, error)
            return False
        votes_ready = True
        return True

    votes_ready = False
    votes_retry_after = 0.0
    ensure_vote_table()

    global vote_table_check
    vote_table_check = ensure_vote_table
    ...
    @app.get("/healthz")
    def healthz():
        # Veritabanına gitmez: Render'ın sağlık kontrolü ve canlı tutma pingi
        # buraya geliyor, yavaşlarsa deploy başarısız sayılabiliyor.
        return jsonify({"ok": True, "data_age_hours": _data_age_hours(allow_stale=True)})
    ...
    def status():
        ...
        return jsonify({
            ...
            "storage": {**store.health(), "votes_ready": votes_ready},
        })
    ...
    def submit_vote():
        ...
        if not ensure_vote_table():
            return jsonify({"error": "Oylama şu anda kullanılamıyor (veritabanına erişilemiyor)."}), 503
