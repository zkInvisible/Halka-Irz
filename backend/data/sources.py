...
class HalkarzSource:
    ...
    def fetch_calendar_entries(self) -> list[dict[str, Any]]:
        """Yalnızca tarihi belli olan takvim kayıtları."""
        return self.fetch_calendar()["scheduled"]
    ...
    def fetch_recent_candidates(self, reference_date: date, lookback_days: int) -> list[dict[str, Any]]:
        ...
        entries, _ = self.fetch_archive_entries(years)
        return entries

    def fetch_draft_entries(self) -> list[dict[str, Any]]:
        """SPK sürecindeki, henüz tarihi açıklanmamış arzlar."""
        return self.fetch_calendar()["drafts"]
