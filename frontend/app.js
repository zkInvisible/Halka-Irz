...
/* ─── Formatters ─── */
/* Tavan serisi süren bir hissede toplam el değiştirme bu oranı aşınca uyarı çıkar. */
const TURNOVER_WARNING_PCT = 20;
...
function renderDistributionBlock(offer) {
  ...
  if (avgLot != null || offer.participant_count != null) {
    ...
    return `
      <div class="dist-card">
        ...
        ${highNote}
        <small title="Eşit dağıtımda küçük talepler dolduktan sonra kalan lotlar büyük taleplere yeniden dağıtılır; kaynak kişi bazındaki en yüksek dağıtımı yayımlamaz.">Ortalamadır;
        yüksek tutarla başvuran birkaç katını alabilir.</small>
      </div>`;
  }
  ...
  if (scenarios.length) {
    ...
    const likelyTip = band?.median
      ? `Son ${band.sample_size} arzın medyanı: ${participantsLabel(band.median)}`
      : 'Son arzların katılımına en yakın senaryo';
    const rows = scenarios.map(s => `
      <div class="dist-row dist-row--proj${s.is_likely ? ' dist-row--likely' : ''}"${s.is_likely ? ` title="${likelyTip}"` : ''}>
    ...
    return `
      <div class="dist-card">
        ...
        <small>Ortalamadır; yüksek tutarla başvuran daha fazlasını alabilir.</small>
      </div>`;
  }
  return '';
}
...
function renderDrawerContent(offer, isHistorical) {
      ...
      similarHtml = `
        <div class="report-section" style="margin-top:20px;border-top:1px solid var(--border);padding-top:20px;">
          ...
          <div style="display:flex;flex-direction:column;gap:8px;">
            ${similar.map(s => {
              const se = outcomeExtreme(s.historical_outcome);
              const peakPct = se.pct;
              const tvn = peakPct != null
                ? `%${fmt(peakPct)}${se.streak ? ` <small style="color:var(--text-tertiary)">${se.streak} ${se.kind}</small>` : ''}`
                : '—';
              return `
  ...
  let hoStrip = '';
  if (ho) {
    const ext = outcomeExtreme(ho);
    const maxTlKar = maxProfitTl(offer);
    const extLabel = ext.kind === 'taban'
      ? (ext.isExit ? 'İlk Çıkış (%)' : 'Dip (%)')
      : 'Zirve (%)';
    const extSub = ext.streak
      ? `<em>${ext.streak} ${ext.kind}${ext.isExit ? ` · ${ext.session}. seans` : ''}</em>`
      : '';
    const tlLabel = ext.kind === 'taban' ? 'Ort. Zarar' : 'Ort. Zirve Kârı';
    ...
    hoStrip = `
      <section class="outcome-strip" data-active="${ho.is_streak_active ? 'true' : 'false'}">
        ...
        <div><span>${extLabel}</span><strong style="color:${returnColor(ext.pct)}">${ext.pct != null ? '%'+fmt(ext.pct) : '—'}</strong>${extSub}</div>
        ${ho.is_streak_active ? `<div><span>Toplam El Değişim</span><strong style="color:${ho.latest_turnover_pct >= TURNOVER_WARNING_PCT ? 'var(--red)' : 'var(--text-primary)'}">${ho.latest_turnover_pct != null ? '%'+fmt(ho.latest_turnover_pct) : '—'}</strong></div>` : ''}
        <div><span>${tlLabel}</span><strong style="color:${returnColor(maxTlKar)}">${maxTlKar != null ? (maxTlKar >= 0 ? '+' : '') + fmt(Math.round(maxTlKar))+'&nbsp;₺' : '—'}</strong></div>
        <div><span>Arz→Bugün</span><strong style="color:${returnColor(ho.return_since_ipo_pct)}">${fmtPct(ho.return_since_ipo_pct)}</strong>${ho.split_factor_since_listing ? `<em>1 lot → ${fmt(Math.round(ho.split_factor_since_listing*100)/100)} lot</em>` : ''}</div>
        <p>Puan yalnızca arz tarihinden önceki gözlemlerle hesaplanmıştır.${ho.split_method ? ` ${ho.split_method}` : ''}</p>
      </section>`;
  }
...
function getFilteredHistory() {
  ...
  const extreme = (o) => outcomeExtreme(o.historical_outcome).pct ?? -Infinity;
  if (sk === 'tavan-desc')  offers.sort((a,b) => extreme(b) - extreme(a));
  if (sk === 'tavan-asc')   offers.sort((a,b) => extreme(a) - extreme(b));
...
/* Arzın gerçekçi sonucu.

   Tavan ve taban simetrik değildir: tavanda alıcı yığıldığı için satabilirsin,
   tabanda alıcı olmadığı için satamazsın.

   - Kâra geçtiyse → zirve (tavanda satılabildiği için ulaşılabilir bir fiyat).
   - Hiç kâra geçmediyse → taban serisinin kırıldığı seansın kapanışı, yani
     yatırımcının ilk çıkabildiği fiyat. Dibi göstermek yanıltıcı olur; dip
     çoğu zaman çıkış imkânı doğduktan çok sonra oluşuyor (ENDAE: ilk çıkış
     %-2,8 iken dip 26. seansta %-35,3).
   - Hâlâ taban serisindeyse → çıkış henüz mümkün değil. */
/* Bedelsiz sermaye artırımı yapmış hisselerde bugünkü fiyat, arz fiyatıyla
   doğrudan kıyaslanamaz: elde tutulan lot adedi artmıştır. Fiyatın yanına
   kaç lota çıktığını yazmazsak "arz 15,30 ₺ · bugün 14,02 ₺ · getiri %+424"
   satırı tutarsız görünür. */
function splitBadge(outcome) {
  const factor = outcome && outcome.split_factor_since_listing;
  if (!factor || factor <= 1) return '';
  const events = (outcome.split_events || []).map(event => {
    const [year, month, day] = String(event.date || '').split('-');
    const when = day ? `${day}.${month}.${year}` : event.date;
    return `${when} (${fmt(Math.round(event.ratio * 100) / 100)}×)`;
  }).join(', ');
  const rounded = fmt(Math.round(factor * 100) / 100);
  return `<sup class="split-badge" title="Bedelsiz sermaye artırımı: ${events}. Arzda alınan 1 lot bugün ${rounded} lot; arz→bugün getirisi bu artışa göre hesaplandı.">×${rounded}</sup>`;
}

function outcomeExtreme(outcome) {
  if (!outcome) return { pct: null, streak: 0, kind: null, session: null };
  const peak = outcome.peak_return_pct;

  if (peak == null) {
    const fallback = maxGainPct(outcome).pct;
    return { pct: fallback, streak: outcome.max_limit_up_streak || 0, kind: 'tavan', session: null };
  }
  if (peak > 0) {
    return {
      pct: peak,
      streak: outcome.max_limit_up_streak || 0,
      kind: 'tavan',
      session: outcome.peak_session,
    };
  }
  if (outcome.still_limit_down_locked) {
    return {
      pct: null, streak: outcome.opening_limit_down_streak || 0,
      kind: 'taban', session: null, locked: true,
    };
  }
  if (outcome.first_sellable_return_pct != null) {
    return {
      pct: outcome.first_sellable_return_pct,
      streak: outcome.opening_limit_down_streak || 0,
      kind: 'taban',
      session: outcome.first_sellable_session,
      isExit: true,
    };
  }
  // Açılışta taban yoksa hisse ilk günden satılabilirdi; en kötü nokta diptir.
  return {
    pct: outcome.trough_return_pct ?? peak,
    streak: outcome.max_limit_down_streak || 0,
    kind: 'taban',
    session: outcome.trough_session,
  };
}

/* Ortalama katılımcının uç noktadaki kâr/zararı. Hiç kâra geçmemiş arzda bu
   sayı negatiftir: taban gidildiyse kayıp da aynı biçimde gösterilir. */
function maxProfitTl(offer) {
  const tl = investedTl(offer);
  const { pct } = outcomeExtreme(offer.historical_outcome);
...
function renderHistory() {
  ...
  visible.forEach(offer => {
    ...
    const ext = outcomeExtreme(outcome);
    const maxTavanPct = ext.pct;
    const streakBadge = ext.streak
      ? `<br><small style="color:var(--text-tertiary);font-size:11px">${ext.streak} ${ext.kind}</small>`
      : '';
    const extTitle = ext.locked
      ? 'Taban serisi sürüyor, satış imkânı doğmadı'
      : ext.isExit
        ? `${ext.streak} taban sonrası ${ext.session}. seansta ilk satış imkânı`
        : ext.session
          ? `${ext.session}. seansta ${ext.kind === 'taban' ? 'dip' : 'zirve'}`
          : '';
    const maxTavanText = ext.locked
      ? `<span title="${extTitle}" style="color:var(--red)">satılamıyor</span>${streakBadge}`
      : maxTavanPct != null
        ? `<span title="${extTitle}">%${fmt(maxTavanPct)}</span>${streakBadge}`
        : '—';
    ...
    // Uyarı yalnızca tavan serisi sürerken anlamlı: seri bozulduktan sonra
    // "yakında bozulabilir" demek bilgi taşımaz. is_streak_active bunu sağlıyor.
    let warningIcon = '';
    if (outcome.is_streak_active && outcome.latest_turnover_pct >= TURNOVER_WARNING_PCT) {
      warningIcon = `<span class="warning-icon" title="Dikkat! Toplam el değiştirme oranı çok yüksek (%${fmt(outcome.latest_turnover_pct)}), tavan serisi yakında bozulabilir.">🚨</span>`;
    }

    row.innerHTML = `
      <td style="color:var(--accent);font-weight:700;font-family:'JetBrains Mono',monospace">${offer.ticker}${warningIcon}</td>
      <td>${offer.company || '—'}</td>
      <td style="text-align:right;font-weight:600;padding-right:25px;color:var(--text-primary);white-space:nowrap">${outcome.latest_close_tl ? fmt(outcome.latest_close_tl) + '&nbsp;₺' + splitBadge(outcome) : '—'}</td>
